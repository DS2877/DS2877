...
### D-030 · The Laser Gate moved beside the path, and shares the bottom bar's third slot
**Decision.** The gate button now stands beside the base path, mirroring the egg nest across it, and the bottom bar's third button offers **LASER GATE** when you are standing in your own base with the gate down — the same slot that shows **SAVE IT!** when a thief is near.
**Why the move.** The button was at `z = BASE_WIDTH/2 - 7`: 47 studs along the road from where you spawn, in a corner of a 108 × 76 plot, with a 12-stud activation range. *"Activating the laser gate"* was one of three things a playtester said did not work, and the most likely reading is that he never got near enough to see a prompt at all. A defence you have to go looking for is not a defence.
**Why it shares a slot.** The rail cannot take a fifth round button — 5 × 44 + 4 × 6 is 244pt into 209pt of rail on a 375pt screen, and a `UIListLayout` does not complain, it puts the last one off the bottom edge. `tests/unit/hud.spec.luau` now pins that. Sharing costs nothing: SAVE and LASER GATE are both defence and cannot matter in the same instant, and the same playtest called the HUD *"way too much"*.
**This does not weaken D-025.** The client decides what to OFFER; `GateService.raise` re-checks proximity, ownership and cooldown from scratch. The client's own check is deliberately the more generous of the two, so the button appears a step early and then explains itself rather than refusing to appear and teaching nothing.
