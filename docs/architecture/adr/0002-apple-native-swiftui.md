...
Build all Apple clients (iPhone, iPad, Mac, Apple TV) natively in **Swift + SwiftUI**. Do not use React Native, Flutter, or any WebView-based UI for the core experience.
...
- Apple TV's focus-engine interaction model and tvOS's NetworkExtension constraints (whole-tunnel only, min tvOS 17 — see `/docs/research/apple-platform-requirements.md`) are best handled with native tvOS SwiftUI, not a generic cross-platform renderer.
...
- A shared Swift package structure (`AeriaCore`, `AeriaNetworking`, `AeriaModels`, `AeriaDesignSystem`, `AeriaVPN`, `AeriaAuthentication`, `AeriaSubscriptions`) shares business logic and design tokens across iOS/iPadOS/macOS/tvOS without sharing UI rendering — see `/docs/architecture/ARCHITECTURE.md`.
